...
<g id="Camada-1" serif:id="Camada 1" fill-rule="nonzero">
  ...
  <defs>
	...
	function __construct( $plugin ) {
		...
		// skip any admin related stuff when doing ajax, cron or rest
		...
		add_action( 'admin_post_nopriv_wpb_clear_cache', [ $this, 'admin_clear_cache' ] );
		add_action( 'admin_post_wpb_repair_install', [ $this, 'admin_repair_installation' ] );
	...
	protected function getJsTraslations() {
		return [
			...
			'no_cache_known_users_description'     => __( 'This disables cache for logged in users or visitors who left comments on posts.', 'ezcache' ),
			...
			'cache_clear_on_post_edit_description' => __( 'Make sure your visitors read the latest posts by clearing the cache when you update or publish a post.', 'ezcache' ),
	...
	function maybe_show_advanced_cache_notice() {
		...
		if ( isset( $_SESSION['EZCACHE_REPAIRED'] ) ) {
			unset( $_SESSION['EZCACHE_REPAIRED'] );
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'ezCache installation repaired.', 'ezcache' ) . '</p></div>';
		...
		if ( ! $status['cache_enabled'] ) {
			$messages[] = __( 'WP_CACHE is disabled in your wp-config.php', 'ezcache' );
		}
		if ( ! $status['adv_cache_exists'] ) {
			$messages[] = __( 'advanced-cache.php is missing from your wp-content folder', 'ezcache' );
		}
		if ( $status['adv_cache_exists'] && ! $status['correct_cache_exists'] ) {
			$messages[] = __( 'Invalid advanced-cache.php file detected', 'ezcache' );
		}
		...
		esc_html_e( 'ezCache detected the following configuration problems:', 'ezcache' );
		echo '<br>';
		echo implode( ', ', $messages );
		echo '<br>';
		echo '<a class="button button-sm button-secondary" href="' . wp_nonce_url( admin_url( 'admin-post.php?action=wpb_repair_install&_wp_http_referer=' . $referer ), 'wpb-repair-install' ) . '">' . esc_html__( 'Repair ezCache Installation', 'ezcache' ) . '</a>';
		echo '</p></div>';
	}
	 ...
	 */
	function admin_repair_installation() {
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'wpb-repair-install' ) ) {
			wp_nonce_ays( 'wpb-repair-install' );
		}
